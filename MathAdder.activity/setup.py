...
class Builder(object):
    ...
    def build_locale(self):
        ...
        for f in os.listdir(po_dir):
            ...
            localedir = os.path.join(source_dir, 'locale', lang)
            mo_path = os.path.join(localedir, 'LC_MESSAGES')
            if not os.path.isdir(mo_path):
                os.makedirs(mo_path)

            mo_file = os.path.join(mo_path, "%s.mo" % bundle_id)
...
class XOPackager(Packager):
    ...
    def package(self):
        ...
        for f in self.builder.get_files():
            bundle_zip.write(os.path.join(source_dir, f).strip(),
                             os.path.join(bundle_name, f.strip()))
        bundle_zip.close()
